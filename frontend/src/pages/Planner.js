    ...
    <div className="relative inline-block sm:hidden">
      ...
      {mobileMenuOpen && (
        <div className="absolute right-0 mt-2 py-2 w-48 bg-white shadow-lg rounded-xl">
          <button
            className={`block px-4 py-2 text-gray-800 hover:bg-gray-200 ${activeTab === 0 ? 'font-bold' : ''}`}
            onClick={() => handleTabChange(0)}
          >
            My Routines
          </button>
          <button
            className={`block px-4 py-2 text-gray-800 hover:bg-gray-200 ${activeTab === 1 ? 'font-bold' : ''}`}
            onClick={() => handleTabChange(1)}
          >
            My Favourites
          </button>
          <button
            className={`block px-4 py-2 text-gray-800 hover:bg-gray-200 ${activeTab === 2 ? 'font-bold' : ''}`}
            onClick={() => handleTabChange(2)}
          >
            Other Routines
          </button>
